...
parser.add_argument('--bs', type=int, default=2, help='training batch size')
...
parser.add_argument('--img_flist', type=str, default='train_images.flist')
parser.add_argument('--mask_flist', type=str, default='mask.flist')
parser.add_argument('--model_type', type=str, default='U2_0')
parser.add_argument('--threshold', type=float, default=0.8)
parser.add_argument('--pretrained_sr', default='./data/weights/U2_0/x_labd346-MS-7B17U2_0_U2_0_bs_2_epoch_5.pth', help='pretrained base model')
parser.add_argument('--pretrained', type=bool, default=False)
parser.add_argument('--save_folder', default='./data/weights/', help='Location to save checkpoint models')
parser.add_argument('--prefix', default='U2_0', help='Location to save checkpoint models')
...
parser.add_argument('--gan_weight', type=float, default=0.2)
...
parser.add_argument('--test_mask_flist', type=str, default='mask0.1-0.2.flist')
parser.add_argument('--test_img_flist', type=str, default='test_images.flist')
...
def train(epoch):
    ...
    for batch in training_data_loader:
        gt, mask, index, _ = batch
        t_io2 = time.time()
        if cuda: #将图片放入cuda
            gt = gt.cuda()
            mask = mask.cuda()

        prediction = model.generator(gt, mask) #prediction为完全重绘图片 = d3
        merged_result = prediction * mask + gt * (1 - mask)#仅使用mask部分的重绘结果，其他部分沿用原图像   这个为输出图片
        ...
        d_fake, _ = model.discriminator(merged_result.detach())


        d_real = (torch.mean(F.relu(mask - d_real)) / torch.mean(mask) + torch.mean(F.relu(d_real - mask)) / torch.mean(1 - mask)) / 2.0
        #经过
        d_fake = (torch.mean(F.relu(mask - d_fake)) / torch.mean(mask) + torch.mean(F.relu(d_fake - mask)) / torch.mean(1 - mask)) / 2.0

        d_real_loss = model.adversarial_loss(d_real, True, True) #第一个为输入的值，第二个为是否是真实数据，第三个为是否是来自判别器的输入值
        d_fake_loss = model.adversarial_loss(d_fake, False, True)
        d_loss += (d_real_loss + d_fake_loss) / 2

        g_fake, _ = model.discriminator(merged_result)

        g_fake = (torch.mean(F.relu(mask - g_fake)) / torch.mean(mask) + torch.mean(F.relu(g_fake - mask)) / torch.mean(1-mask))/2.0
        ...
        gt_sobel = model.sobel(gt)
        # gt_sobel_stage1 = model.sobel_stage1(gt)
        result_sobel = model.sobel(merged_result)
        g_sobel_loss = 0
        for i in range(2):
            g_sobel_loss += model.l1_loss(gt_sobel[i], result_sobel[i])

        g_sobel_loss = g_sobel_loss / torch.mean(mask)
        g_loss += g_sobel_loss
        ...
        if iteration % opt.print_interval == 0:
            print("=> Epoch[{}]({}/{}): Avg L1 loss: {:.6f} | G loss: {:.6f} | Avg D loss: {:.6f} || Timer: {:.4f} sec. | IO: {:.4f} || G GAN loss: {:.6f}".format(
                epoch, iteration, len(training_data_loader), avg_l1_loss/opt.print_interval, avg_g_loss/opt.print_interval,
                avg_d_loss/opt.print_interval, td, t_io2-t_io1, avg_gan_loss/opt.print_interval), flush=True)
...
def test(gen, dataloader):
    ...
    for batch in dataloader:
        gt_batch, mask_batch, index, _ = batch
        if cuda:
            gt_batch = gt_batch.cuda()
            mask_batch = mask_batch.cuda()
        with torch.no_grad():
            pred_batch, _ = model.generator(gt_batch, mask_batch)
