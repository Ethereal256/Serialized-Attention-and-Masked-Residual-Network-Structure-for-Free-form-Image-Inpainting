...
from dataset import build_dataloader, Dataset
import pdb
import socket
import time
import skimage
...
parser.add_argument('--bs', type=int, default=10, help='training batch size')
...
parser.add_argument('--gpus', default=2, type=int, help='number of gpu during train')
parser.add_argument('--threshold', type=float, default=0.8)
parser.add_argument('--img_flist', type=str, default='test_images.flist')
parser.add_argument('--mask_flist', type=str, default='mask0.4-0.5.flist')
parser.add_argument('--model', default='./data/weights/U2_0/x_labd346-MS-7B17U2_0_U2_0_bs_2_epoch_15.pth', help='sr pretrained base model')
parser.add_argument('--save', default=True, action='store_true', help='If save test images')
parser.add_argument('--save_path', type=str, default='./test_results/20')
...
def SSIM(pred, gt, data_range=255, win_size=11, multichannel=True):
    ...
    return ssim(pred, gt, data_range=data_range, \
...
def evaluate_batch(batch_size, gt_batch, pred_batch, mask_batch, save=False, path=None, count=None, index=None, name=None):
    ...
    for i in range(batch_size):
        ...
        gt, pred, fname = gt_batch[i], pred_batch[i], name[i]

        psnr += PSNR(pred, gt)
        ssim += SSIM(pred, gt)
        l1 += L1(pred, gt)

        # name = Dataset.load_name(i)
        ...
        if save:
            ...
            # ---------------------------------------------------------------------
            save_img(path, str(fname).replace(".jpg", ".png"), pred_batch[i])
            # save_img(path, str(fname).replace(".jpg", "_input.png"), input_batch[i])
            # save_img(path, str(fname).replace(".jpg", "_mask.png"), mask_batch[i])
            # save_img(path, str(fname).replace(".jpg", "_gt.png"), gt_batch[i])
            #------------------------------------------------------------------------------

            # save_img(path, str(fname), pred_batch[i])
            # save_img(path, str(fname).replace(".jpg", "_input.jpg"), input_batch[i])
            # save_img(path, str(fname).replace(".jpg", "_mask.jpg"), mask_batch[i])
            # save_img(path, str(fname).replace(".jpg", "_gt.jpg"), gt_batch[i])
