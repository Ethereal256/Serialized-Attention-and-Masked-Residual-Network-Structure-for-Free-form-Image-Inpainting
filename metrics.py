...
for fn in sorted(files):
    ...
    img_pred = (imread(path_pred + '/' + basename(str(fn).replace('jpg','png'))) / 255.0).astype(np.float32)

    img_gt1 = torch.from_numpy(img_gt)
    img_gt1 = img_gt1.unsqueeze(0).transpose(1,3).transpose(2,3)
    img_pred1 = torch.from_numpy(img_pred)
    img_pred1 = img_pred1.unsqueeze(0).transpose(1, 3).transpose(2, 3)

    d = loss_fn_alex(img_gt1, img_pred1).item()
    # print(d)
    lpips1.append(d)
    ...
    if np.mod(index, 100) == 0:
        print(
            ...
            "LPIPS: %.4f" % round(np.mean(lpips1), 4),
        )
    index += 1
